...
vec3 CalculatePointLight(PointLight point_light, vec3 normal)
{
	...
	// DIFFUSE
	float diffuse_intensity = point_light.intensity * max(0, dot(L, normal));
...
vec3 CalculateSpotLight(SpotLight spot_light, vec3 normal)
{
	...
	if(varyingcos > spot_light.cosangle)
	{
		...
		float intensity = spot_light.base.intensity * (1.0f - (1.0f - varyingcos) / (1.0f - spot_light.cosangle));
...
void main(void)
{
	...
	Result = Result * texture(sampler_tex, varying_uv_coord).rgb;
